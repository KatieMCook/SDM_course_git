...
setwd("D:/corona_contingency/SDM_course_git/mols_predictions_jan")

#stack

ens_list_stack<- stack(ens_mask)
...
for(i in 1:length(ens_list_stack)){
  ...
  writeRaster(towrite, paste0('molls_pred_now',i,'.tif'))
}
...
for(i in 1:length(ens_85_stack)){
  ...
  writeRaster(towrite, paste0('molls_pred_RCP85',i,'.tif'))
}
...
for(i in 1:length(ens_26_stack)){
  ...
  writeRaster(towrite, paste0('molls_pred_RCP26',i,'.tif'))
}
...
for(i in 1:length(dif_site_list)){
  dif_site_list[[i]]$group<-paste0('mollusc', i)
}
...
#now write csv
write.csv(dif_site_all, 'dif_site_mollusc.csv')
