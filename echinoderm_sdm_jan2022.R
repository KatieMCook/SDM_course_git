...
setwd("D:/corona_contingency/SDM_course_git/echino_predictions_jan")
#stack groups
#for present 
ens_stack<-stack(ens_mask)
plot(ens_stack)

#change to the new string 
crs(ens_stack)<-CRS(SRS_string = "EPSG:4326")

#now write 
for(i in 1:length(ens_stack)){
  towrite<-ens_stack[[i]]
  writeRaster(towrite, paste0('echino_pred_now_',i,'.tif'))
}
...
for(i in 1:length(ens_85_stack)){
  ...
  writeRaster(towrite, paste0('echino_pred_RCP85_',i,'.tif'))
}
...
for(i in 1:length(ens_26_stack)){
  ...
  writeRaster(towrite, paste0('echino_pred_RCP26_',i,'.tif'))
}
