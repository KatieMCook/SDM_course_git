...
#now read in cleaned algae data
algae_all<-read.csv('algae_site_FG.csv')
...
#plot proportions 
site_prop$group<-as.factor(site_prop$group)
...
#for tropical vs subtropical sp.

par(mfrow=c(1,2))
plot(ens_list[[1]], main='Group 1', col=pal)
plot(japan_outline, add=TRUE, col='grey', border='black')
box()

plot(ens_list[[3]], main='Group 3', col=pal)
plot(japan_outline, add=TRUE, col='grey', border='black')
box()
...
for (i in 1:length(dif_list)){
  plot(dif_list[[i]], main=i, col=pal)
  plot(japan_outline, col='light grey', add=TRUE)
  box()
}

par(mfrow=c(1,2))
plot(dif_list[[1]], main='Group 1', col=pal)
plot(japan_outline, add=TRUE, col='light grey')
box()
plot(dif_list[[3]], main='Group 3', col=pal)
plot(japan_outline, col='light grey', add=TRUE)
box()
#to keep the scale the same #nah looks bad 


stack_posterplot<-stack(dif_list)

par(mfrow=c(3,2),  mai = c(0.3,0.2,0.2,0.2))

p1<-spplot(stack_posterplot, col.regions=pal)
p1
...
par(mfrow=c(1,1))
increase_trop_stack<-stack(increase_list[3])
plot(increase_trop_stack)

increase_subtrop_stack<-stack(increase_list[c(1,2,4,5)])
plot(increase_subtrop_stack)

decrease_trop_stack<-stack(decrease_list[3])
plot(decrease_trop_stack)

decrease_subtrop_stack<-stack(decrease_list[c(1,2,4,5)])
...
subtrop_decrease<-subtrop_decrease 

plot(subtrop_decrease)


pal <- viridis(n=40, option='plasma' )
palm<-viridis(n=40, option='plasma' ,  direction=-1)
...
par(mfrow=c(1,2))
plot(trop_decrease, col=pal, main='Tropical FG Decrease')
plot(japan_outline, add=TRUE, col='light grey')
box()
plot(subtrop_decrease, col=pal,  main='Subtropical FG Decrease')
plot(japan_outline, add=TRUE, col='light grey')
box()
...
writeRaster(trop_increase, 'change_hotspots/algae_trop_increase.tif', format='GTiff')
writeRaster(sub_trop_increase, 'change_hotspots/algae_subtrop_increase.tif', format='GTiff')
writeRaster(trop_decrease, 'change_hotspots/algae_trop_decrease.tif', format='GTiff')
writeRaster(subtrop_decrease, 'change_hotspots/algae_subtrop_decrease.tif', format='GTiff')
